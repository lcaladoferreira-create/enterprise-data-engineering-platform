...
def create_gold_star_schema(spark: SparkSession) -> None:
    ...
    Uses Delta Lake format for all outputs.
    """
    try:
        ...
        order_items = spark.read.format("delta").load(f"{config.SILVER_PATH}/order_items")
        products = spark.read.format("delta").load(f"{config.SILVER_PATH}/products")
        payments = spark.read.format("delta").load(f"{config.SILVER_PATH}/payments")
        invoices = spark.read.format("delta").load(f"{config.SILVER_PATH}/invoices")

        # 1. Dimension: dim_customers
        dim_customers = customers.select("customer_id", "first_name", "last_name", "city", "country").withColumnRenamed(
            "customer_id", "customer_key"
        )
        dim_customers.write.format("delta").mode("overwrite").save(f"{config.GOLD_PATH}/dim_customers")

        # 2. Dimension: dim_products
        dim_products = products.select("product_id", "name", "category", "brand").withColumnRenamed(
            "product_id", "product_key"
        )
        dim_products.write.format("delta").mode("overwrite").save(f"{config.GOLD_PATH}/dim_products")
        ...
        if orders.limit(1).count() > 0:
            ...
            if min_date and max_date:
                ...
                dim_dates = (
                    ...
                    .select(F.expr(f"date_add('{min_date.date()}', cast(id as int))").alias("date_key"))
                    .select(
                        "date_key",
                ...
                dim_dates.write.format("delta").mode("overwrite").save(f"{config.GOLD_PATH}/dim_dates")
        else:
        ...
        fact_orders.write.format("delta").mode("overwrite").save(f"{config.GOLD_PATH}/fact_orders")
        ...
        fact_payments.write.format("delta").mode("overwrite").save(f"{config.GOLD_PATH}/fact_payments")

        # 6. Data Mart: Daily Revenue
        mart_sales_daily = fact_orders.groupBy("order_date_key").agg(
            F.count("order_id").alias("total_orders"), F.sum("total_amount").alias("revenue")
        )
        mart_sales_daily.write.format("delta").mode("overwrite").save(f"{config.GOLD_PATH}/mart_sales_daily")

        # 7. Data Mart: Customer Lifetime Value (CLV)
        mart_clv = fact_orders.groupBy("customer_key").agg(
            F.sum("total_amount").alias("lifetime_spend"),
            F.count("order_id").alias("total_orders"),
            F.min("order_date_key").alias("first_purchase_date"),
        )
        mart_clv.write.format("delta").mode("overwrite").save(f"{config.GOLD_PATH}/mart_customer_lifetime_value")
        ...
        mart_prod_perf.write.format("delta").mode("overwrite").save(f"{config.GOLD_PATH}/mart_product_performance")

        # 9. Data Mart: Payment Variance Analysis
        invoices.join(payments, "invoice_id", "left").select(
            invoices.invoice_id,
            invoices.order_id,
        ...
        ).write.format("delta").mode("overwrite").save(f"{config.GOLD_PATH}/mart_payment_reconciliation")

        logger.info("Gold layer materialization complete.")

    except Exception as e:
        logger.error(f"Gold layer materialization failed: {str(e)}")
        raise
